...
Inventory also has a scheduled reconciliation path through Product's internal `ProductSnapshotService/ListInventorySnapshots` gRPC contract. It pages every authoritative product, provisions/synchronizes inventory metadata, and retires absent inventory rows while preserving counters. This supplements Kafka; it does not replace normal lifecycle event delivery.
