...
function RecordTools({ query, onQuery, count, total, loading, onReload, noun = "records" }: { query: string; onQuery: (value: string) => void; count: number; total: number; loading: boolean; onReload: () => void; noun?: string }) {
  return <div className="bo-record-tools">
    <label className="bo-record-search"><Search size={18} aria-hidden="true" /><span className="sr-only">Search loaded {noun}</span><input type="search" value={query} maxLength={200} placeholder={`Search loaded ${noun}…`} onChange={event => onQuery(event.target.value)} />{query ? <button type="button" aria-label="Clear search" onClick={() => onQuery("")}><X size={16} aria-hidden="true" /></button> : null}</label>
    <div className="bo-record-meta"><span role="status">{loading ? "Loading records…" : query ? `${count} matches among loaded ${noun}` : `${count} loaded / ${total} total`}</span><Button variant="secondary" loading={loading} onClick={onReload}><RefreshCw size={14} aria-hidden="true" />Refresh</Button></div>
  </div>;
}
...
export function AdminUsersPage() {
  const { accessToken } = useAuth();
  const [pageSearch, setPageSearch] = useState("");
  const fetchPage = useCallback((page: number) => api.admin.users(accessTokenOrThrow(accessToken), `page=${page}&size=20`).then(toPage<User>), [accessToken]);
  const records = useInfinitePage(fetchPage);
  const { items, totalElements, loading, error, reload } = records;
  const visibleRecords = filterWorkspaceRecords(items, pageSearch, record => [record.name, record.email, record.id, record.status, record.roles, record.role]);
  return <section className="backoffice-page"><div className="page-heading"><div><span className="eyebrow">Identity operations</span><h1>Users</h1><p>Manage account access, roles, and status from one place.</p></div></div><ContractNote>The available API is paginated but does not publish search or filter semantics. Use a known user ID for a detailed record; browser permissions never replace server authorization.</ContractNote><RecordTools noun="users" query={pageSearch} onQuery={setPageSearch} count={visibleRecords.length} total={totalElements} loading={loading} onReload={() => void reload()} />{loading ? <LoadingBlock label="Loading users" /> : error ? <PageError message={error} retry={() => void reload()} /> : items.length ? <><div className="table-wrap"><table className="data-table"><thead><tr><th>Name</th><th>Email</th><th>Roles</th><th>Status</th><th><span className="sr-only">Open user</span></th></tr></thead><tbody>{!visibleRecords.length ? <tr><td colSpan={5}><div className="bo-table-empty"><span>No matches among loaded records. Load more records or clear search.</span><Button variant="secondary" onClick={() => setPageSearch("")}>Clear search</Button></div></td></tr> : null}{visibleRecords.map((record) => <tr key={record.id}><td><strong>{record.name}</strong><br /><code>{record.id.slice(0, 8)}</code></td><td>{record.email}</td><td>{(record.roles?.length ? record.roles : [record.role]).join(", ")}</td><td><StatusBadge value={record.status} /></td><td><Link className="button button-ghost" to={`/admin/users/${encodeURIComponent(record.id)}`}>Open</Link></td></tr>)}</tbody></table></div><InfiniteListFooter {...records} loadedCount={items.length} noun="users" /></> : <EmptyState title="No users returned" message="No user records were returned." />}</section>;
}
