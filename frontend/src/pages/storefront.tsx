...
function CartLine({ item, product }: { item: CartItem; product?: Product }) {
  ...
  return <article className="cart-line"><ProductImage product={product ?? { id: item.productId, name: "Product", price: 0 }} compact /><div className="cart-line-info"><strong>{product?.name ?? "Product details are unavailable"}</strong><span>{product ? formatMoney(product.price, product.currency) : "Price confirmed at checkout"}</span>{error ? <span className="field-error">{error}</span> : null}</div><div className="cart-line-actions"><div className="quantity-control"><Button variant="secondary" disabled={busy || item.quantity <= 1} onClick={() => void change(item.quantity - 1)}>−</Button><output>{item.quantity}</output><Button variant="secondary" disabled={busy || item.quantity >= cartMaxQuantity} onClick={() => void change(item.quantity + 1)}>+</Button></div><Button variant="ghost" disabled={busy} onClick={() => void deleteItem()}>Remove</Button></div></article>;
}

export function CartPage() {
  const { cart, isLoading, error, mergeState, refresh, mergeGuest } = useCart();
  ...
  return <section className="cart-page">
    <div className="page-heading"><div><span className="eyebrow">{isAuthenticated ? "Signed-in cart" : "Guest cart"}</span><h1>Your cart</h1></div><SafeLink to="/">Continue shopping</SafeLink></div>
    {mergeState === "merging" ? <Alert title="Bringing your cart together">Your guest cart is being merged securely. Please do not refresh.</Alert> : null}
    {mergeState === "retry" ? <Alert tone="warning" title="Cart merge needs attention" action={<Button variant="secondary" onClick={() => void mergeGuest()}>Retry merge</Button>}>We could not finish bringing in your guest items. Retry the merge to check them again.</Alert> : null}
    {error ? <PageError message={error} retry={() => void refresh()} /> : null}
    {error && !cart ? null : cart?.items.length ? <div className="cart-layout">
      <div className="cart-lines">{cart.items.map((item) => <CartLine key={item.itemId} item={item} product={productMap.get(item.productId)} />)}</div>
      <aside className="summary-card"><h2>Order summary</h2><div><span>Estimated items total</span><strong>{formatMoney(estimate)}</strong></div><p>Final prices and availability are confirmed at checkout.</p>{isAuthenticated ? <Link className="button button-primary wide-button" to="/checkout">Proceed to checkout</Link> : <><Link className="button button-primary wide-button" to="/login" state={{ from: "/checkout" }}>Sign in to check out</Link><p className="muted">Your bag comes with you when you sign in.</p></>}</aside>
    </div> : <EmptyState title="Your cart is empty" message="Find something you like, then add it here." action={<Link className="button button-primary" to="/">Browse products</Link>} />}
  </section>;
}
...
const cartMaxQuantity = Number(import.meta.env.VITE_CART_MAX_QUANTITY_PER_ITEM ?? "100");
