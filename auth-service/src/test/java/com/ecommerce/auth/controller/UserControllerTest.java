...
class UserControllerTest {
    ...
    void shouldGetMe() {
        ...
        when(userService.getMe(USER_ID))
                .thenReturn(response);

        UserProfileResponse result =
                userController.getMe(jwt());
    ...
    void shouldUpdateMe() {
        ...
        when(userService.updateMe(USER_ID, request))
                .thenReturn(response);

        UserProfileResponse result =
                userController.updateMe(jwt(), request);

        assertThat(result)
                .isSameAs(response);

        verify(userService)
                .updateMe(USER_ID, request);
    }
