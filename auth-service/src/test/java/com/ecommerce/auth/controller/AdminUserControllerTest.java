...
class AdminUserControllerTest {
  ...
  void shouldGetAllUsers() {
    ...
    AdminUserResponse userResponse = mock(AdminUserResponse.class);
    ...
    Page<AdminUserResponse> result = adminUserController.getAllUsers(0, 20);
  ...
  void shouldGetUserById() {
    ...
    AdminUserResponse result = adminUserController.getUserById(USER_ID);
  ...
  void shouldChangeRole() {
    ...
    when(userService.changeRole(USER_ID, request)).thenReturn(response);

    AdminUserResponse result = adminUserController.changeRole(USER_ID, request);

    assertThat(result).isSameAs(response);

    verify(userService).changeRole(USER_ID, request);
  }
  ...
  void shouldForceLogout() {
    ...
    verify(userService).forceLogout(USER_ID);
  }
}
